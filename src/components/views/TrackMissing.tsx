...
export function TrackMissing({
  ...
  const unimplementedExercises = useMemo(() => {
    ...
    return canonical.list!.filter(
      (item) => lookup[item] === undefined && foregone.indexOf(item) === -1
    )
  ...
  return (
    <section>
      <header>
        ...
        <p>
          This is the list of exercises as found in the{' '}
          <ProblemSpecLink>problem specifications</ProblemSpecLink> but not
          implemented in the <code>{trackId}</code> track. As maintainer, if you
          want an exercise to <em>never</em> be implemented, add its{' '}
          <code>slug</code> to the <code>foregone</code> key in{' '}
      ...
      {!isShowable ? (
        <div className="alert alert-info">
          <LoadingIndicator>
            Loading tree at <code>{branch}</code> branch. Filtering those with a{' '}
            <code>.deprecated</code> entry.
          </LoadingIndicator>
        </div>
      ) : (
        <MissingExercisesList
          missing={unimplementedExercises}
          data={canonical.data!}
        />
      )}
    </section>
