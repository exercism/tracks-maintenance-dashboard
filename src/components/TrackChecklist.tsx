   ...
   return (
      <aside className='mt-md-4 mb-4 col-md'>
         <ul className='list-group' style={{ whiteSpace: 'nowrap' }}>
            <li className='list-group-item d-flex justify-content-between' ref={trackBlurbRef}>
               Track blurb
               <BlurbIcon
            ...
            <li className='list-group-item d-flex justify-content-between' ref={approveExerciseVisibleRef}>
               Auto approve exercise
               <AutoApproveIcon
            ...
            <li className='list-group-item d-flex justify-content-between' ref={exerciseInCoreRef}>
               Exercises in core
               <CoreIcon
            ...
            <li className='list-group-item d-flex justify-content-between' ref={exerciseWithTopicsRef}>
               Exercises with topics
               <TopicsIcon
                  onToggleDetails={() => setExercisesWithTopicsVisible(prev => !prev)}
                  loading={!done}
...
const BlurbIcon = ({ loading, valid, onToggleDetails, contentVisible }: PreconfiguredIconProps) => (
   <LoadingIconWithPopover active={contentVisible} loading={loading} valid={valid} onToggle={onToggleDetails}>
      <p>
         This check passes if there is a <code>blurb</code> present in the <code>config.json</code> file, which in turn is located at the
         root of the repository. The <code>blurb</code> is a <strong>requirement</strong>.
      </p>
      <AboutConfigJson />
   </LoadingIconWithPopover>
);

const AutoApproveIcon = ({ loading, valid, onToggleDetails, contentVisible }: PreconfiguredIconProps) => (
   <LoadingIconWithPopover active={contentVisible} loading={loading} valid={valid} onToggle={onToggleDetails}>
      <p>
         This check passes if there is at least one exercise with the <code>auto_approve</code> flag switched to <code>true</code>. In
         general, this is the <em>first core</em> exercise of the track, usually <code>hello-world</code>.
      </p>
      <AboutConfigJson />
   </LoadingIconWithPopover>
);

const CoreIcon = ({ loading, valid, contentVisible, onToggleDetails }: PreconfiguredIconProps) => (
   <LoadingIconWithPopover active={contentVisible} loading={loading} valid={valid} onToggle={onToggleDetails}>
      <p>
         This check passes if there is at least one <code>core</code> exercise in the <code>config.json</code>. Having a <em>core</em> track
         is <strong>required</strong>. Having non-core exercises (side exercises) is optional.
      </p>
...
const TopicsIcon = ({ loading, valid, contentVisible, onToggleDetails }: PreconfiguredIconProps) => (
   <LoadingIconWithPopover active={contentVisible} loading={loading} valid={valid} onToggle={onToggleDetails}>
      <p>
