...
export const ContainedPopover = ({
   ...
   <div style={{ position: 'relative', display: 'inline-block' }}>
      <button type='button' style={{ background: 0, border: 0 }} onClick={onToggle}>
         {toggle}
      </button>
...
export const LoadingIconWithPopover = ({ active, onToggle, loading, valid, children }: IconWithPopoverProps) =>
   loading ? (
      <button type='button' style={{ background: 0, border: 0 }}>
         <LoadingIndicator />
      </button>
   ) : (
      <ContainedPopover active={active} onToggle={onToggle} align='right' toggle={<CheckOrCross value={valid} />}>
         {children}
      </ContainedPopover>
   );
